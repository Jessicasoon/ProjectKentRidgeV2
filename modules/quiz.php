...
class Quiz{
	...
	function deleteMulti($memberID){
		...
		if($this->isOwner($memberID)){
			...
			foreach($questionList as $question){
				if($question != ""){
					$this->removeQuestionMulti($question, $memberID);
				}
			}
			
			// remove the results
			$resultList = explode(',', $this->getResultsMulti());
			foreach($resultList as $result){
				if($result != ""){
					$this->removeResultMulti($result, $memberID);
				}
			}
	...
	// delete the quiz HAVE TO CHANGE YL
	function deleteTest($memberID){
		require('quizrooDB.php');
		...
		if($this->isOwner($memberID)){
			...
			foreach($questionList as $question){
				if($question != ""){
					$this->removeQuestionTest($question, $memberID);
				}
			}
			
			// remove the results
			$resultList = explode(',', $this->getResultsTest());
			foreach($resultList as $result){
				if($result != ""){
					$this->removeResultTest($result, $memberID);
				}
			}
	...
	// delete the quiz
	function delete_NOTUSED($memberID){
		require('quizrooDB.php');
		...
		if($this->isOwner($memberID)){
			...
			foreach($questionList as $question){
				if($question != ""){
					$this->removeQuestion_NOTUSED($question, $memberID);
				}
			}
			
			// remove the results
			$resultList = explode(',', $this->getResults_NOTUSED());
			foreach($resultList as $result){
				if($result != ""){
					$this->removeResult_NOTUSED($result, $memberID);
				}
			}
	...
	// create a new result
	function addResultMulti($result_title, $result_description, $result_picture, $memberID){
		require('quizrooDB.php');
		
		// check if is member
		if($this->isOwner($memberID)){
			// Insert the result
			$insertSQL = sprintf("INSERT INTO q_results_multi(`result_title`, `result_description`, `result_picture`, `fk_quiz_id`) VALUES (%s, %s, %s, %d)",
	...
	// create a new result
	function addResult_NOTUSED($result_title, $result_description, $result_picture, $memberID){
		require('quizrooDB.php');
	...
	// create a new result
	function updateResultMulti($result_title, $result_description, $result_picture, $result_id, $memberID){
		require('quizrooDB.php');
		
		// check if is member
		if($this->isOwner($memberID)){
			// Insert the result
			$insertSQL = sprintf("UPDATE q_results_multi SET `result_title` = %s, `result_description` = %s, `result_picture` = %s WHERE `result_id` = %d",
							   htmlentities(GetSQLValueString($result_title, "text")),
							   htmlentities(GetSQLValueString($result_description, "text")),
							   GetSQLValueString($result_picture, "text"),
	...
	// create a new result
	function updateResult_NOTUSED($result_title, $result_description, $result_picture, $result_id, $memberID){
		require('quizrooDB.php');
	...
	// remove a new result
	function removeResultMulti($result_id, $memberID){
		require('quizrooDB.php');
		
		// owner check
		if($this->isOwner($memberID)){
			// delete the result and also check if this results actually belongs to this quiz
			$insertSQL = sprintf("DELETE FROM q_results_multi WHERE `result_id` = %d AND `result_id` IN(%s)", GetSQLValueString($result_id, "int"), $this->getResultsMulti());
			mysql_query($insertSQL, $quizroo) or die(mysql_error());	
			return true;
		}else{
			return false;
	...
	// remove a new result HAVE TO CHANGE YL
	function removeResultTest($result_id, $memberID){
		require('quizrooDB.php');
	...
	// remove a new result
	function removeResult_NOTUSED($result_id, $memberID){
		require('quizrooDB.php');
		
		// owner check
		if($this->isOwner($memberID)){
			// delete the result and also check if this results actually belongs to this quiz
			$insertSQL = sprintf("DELETE FROM q_results WHERE `result_id` = %d AND `result_id` IN(%s)", GetSQLValueString($result_id, "int"), $this->getResults_NOTUSED());
			mysql_query($insertSQL, $quizroo) or die(mysql_error());	
			return true;
		}else{
			return false;
	...
	function removeQuestionMulti($question_id, $memberID){
		require('quizrooDB.php');
		
		// owner check
		if($this->isOwner($memberID)){
			// delete the options and also check if this results actually belongs to this quiz
			$insertSQL = sprintf("DELETE FROM q_options_multi WHERE `fk_question_id` = %d AND `fk_question_id` IN(%s)", GetSQLValueString($question_id, "int"), $this->getQuestions());
			mysql_query($insertSQL, $quizroo) or die(mysql_error());				
			// delete the result and also check if this results actually belongs to this quiz
	...
	// remove a new question HAVE TO CHANGE YL
	function removeQuestionTest($question_id, $memberID){
		require('quizrooDB.php');
	...
	// remove a new question
	function removeQuestion_NOTUSED($question_id, $memberID){
		require('quizrooDB.php');
	...
	// create a option
	function addOptionMulti($option, $result, $weightage, $question, $memberID){
		require('quizrooDB.php');
		// check if is member
		if($this->isOwner($memberID)){
			// insert the option
	...
	// create a option
	function addOption_NOTUSED($option, $result, $weightage, $question, $memberID){
		require('quizrooDB.php');
	...
	// create a option TEST
	function addOptionTest($option, $result, $question, $memberID){
		require('quizrooDB.php');
		...
		// insert the option
			$insertSQL = sprintf("INSERT INTO q_options_test(`option`, `isCorrect`, `fk_question_id`) VALUES (%s, %d, %d)",
						   htmlentities(GetSQLValueString($option, "text")),
						   GetSQLValueString($result, "int"), //cannot insert if isCorrect = 0?
						   GetSQLValueString($question, "int"));
			mysql_query($insertSQL, $quizroo) or die(mysql_error());
	...
	// update an option
	function updateOptionMulti($option, $result, $weightage, $option_id, $memberID){
		require('quizrooDB.php');
		//$one = 1;
		...
		if($this->isOwner($memberID)){
			// insert the option
			$insertSQL = sprintf("UPDATE q_options_multi SET `option`=%s, `fk_result_id`=%d, `option_weightage`=%d WHERE option_id=%d",
						   htmlentities(GetSQLValueString($option, "text")),
						   GetSQLValueString($result, "int"),
						   GetSQLValueString($weightage, "int"),
	...
	// update an option
	function updateOption_NOTUSED($option, $result, $weightage, $option_id, $memberID){
		require('quizrooDB.php');
	...
	// update an option TEST
	function updateOptionTest($option, $result, $option_id, $memberID){
		require('quizrooDB.php');
		
		// check if is member
		if($this->isOwner($memberID)){
			// insert the option
			$insertSQL = sprintf("UPDATE q_options_test SET `option`=%s, `isCorrect`=%d WHERE option_id=%d",
						   htmlentities(GetSQLValueString($option, "text")),
						   GetSQLValueString($result, "int"),
						   GetSQLValueString($option_id, "int"));
			mysql_query($insertSQL, $quizroo) or die(mysql_error());
	...
	// remove an option
	function removeOptionMulti($option_id, $memberID){
		require('quizrooDB.php');
	...
	// remove an option HAVE TO CHANGE YL
	function removeOptionTest($option_id, $memberID){
		require('quizrooDB.php');
	...
	// remove an option
	function removeOption_NOTUSED($option_id, $memberID){
		require('quizrooDB.php');
	...
	function checkPublishTest($mode){
		...
		if($numQuestions != 0){
			...
			foreach($listQuestion as $question){
				// check the number of options for this question
				$numOptions = $this->getOptionsTest($question, "count");
				if($numOptions < $VAR_QUIZ_MIN_OPTIONS){
					$optionState = false;
				}
			}
	...
	// check if quiz is ready to be published
	function checkPublish_NOTUSED(){
		require('variables.php');		
		// check the number of results
		$numResults = $this->getResults_NOTUSED("count");
		// check the number of questions
		$numQuestions = $this->getQuestions("count");
		// check the number of options
		$listQuestion = explode(',', $this->getQuestions());
		...
		if($numQuestions != 0){
			...
			foreach($listQuestion as $question){
				// check the number of options for this question
				$numOptions = $this->getOptions_NOTUSED($question, "count");
				if($numOptions < $VAR_QUIZ_MIN_OPTIONS){
					$optionState = false;
				}
			}
	...
	function getResultsMulti($type = NULL){
		require('quizrooDB.php');
		$query = sprintf("SELECT result_id FROM q_results_multi WHERE fk_quiz_id = %d", $this->quiz_id);
		$getQuery = mysql_query($query, $quizroo) or die(mysql_error());
		$row_getQuery = mysql_fetch_assoc($getQuery);
		$totalRows_getQuery = mysql_num_rows($getQuery);
	...
	// get the list of results belonging to this quiz
	function getResults_NOTUSED($type = NULL){
		require('quizrooDB.php');
	...
	function getOptions_NOTUSED($question_id, $type = NULL){
		require('quizrooDB.php');
	...
	// get the list of options belonging to a question
	function getOptionsMulti($question_id, $type = NULL){
		require('quizrooDB.php');
		$query = sprintf("SELECT option_id FROM q_options_multi WHERE fk_question_id = %d", $question_id);
		$getQuery = mysql_query($query, $quizroo) or die(mysql_error());
		$row_getQuery = mysql_fetch_assoc($getQuery);
		$totalRows_getQuery = mysql_num_rows($getQuery);
	 ...
	 *********************/
	 function quizRecommendation ($quiz_id, $value){
		require('quizrooDB.php');	// database connections
		
		$insertSQL = sprintf("UPDATE q_quizzes SET `isRecommended`= %d WHERE `quiz_id` = %d",
	...
	/**************************************************************************
	 * Modify on 28 Aug: function for adding result for Personality quiz type
	 **************************************************************************/
	function addMultiTypeResult($result_title, $result_description, $result_picture, $fk_quiz_id){
		require('quizrooDB.php');
			$insertSQL = sprintf("INSERT INTO q_results_multi(`result_title`, `result_description`, `result_picture`, `fk_quiz_id`) VALUES (%s, %s, %s, %d)",
							   htmlentities(GetSQLValueString($result_title, "text")),
							   htmlentities(GetSQLValueString($result_description, "text")),
	 ...
	 **************************************************************************/
	 function addMultiTypeOption($option, $result_title, $result_description, $option_weightage, $fk_question_id){
		require('quizrooDB.php');
			...
			$row_result_id = mysql_fetch_assoc($result);
